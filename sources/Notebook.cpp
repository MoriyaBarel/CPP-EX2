...
void Notebook::check_throw_write(const int page, const int row, const int column, Direction direction, const string& to_write) {
    ...
    for (unsigned long i = 0; i < read.size(); i++) {
        if (read.at(i) != '_') {
            throw logic_error("OOPS can't write over non empty space.");
        }
        if (to_write.at(i) < CHAR_LOWER_BOUND || to_write.at(i) > CHAR_UPPER_BOUND) {
            throw invalid_argument("Invalid character");
        }
    }
}

void Notebook::check_throw_erase(const int page, const int row, const int column, Direction direction, const int length) {
    if (page < 0 || row < 0 || column < 0) {
        throw invalid_argument("page/row/column can't be a negative number.");
    }
    if (column >= MAX_ROW_SIZE) {
...
void Notebook::check_throw_read(const int page, const int row, const int column, Direction direction, const int length) {
    if (page < 0 || row < 0 || column < 0) {
        throw invalid_argument("page/row/column can't be a negative number.");
    }
    if (column >= MAX_ROW_SIZE) {
